...
void segregate_string_mark_half(char* pass2, char* mark){
    ...
    for (int j = 0; j<i; j++){
        ...
        sprintf(combine[j], "%s%s", msg1_add, msg2_add);
        printf("combine[%d] = `%s`\n", j, combine[j]);
        // int length = strlen(combine[j]) + n;
        
        // // add string
        // printf("----- add string ----\n"); 
        // char* temp = (char*)malloc(length);
        // strcpy(temp, combine[0]);
        // if (j!=0){
        //     temp = (char*)malloc(length);
        //     sprintf(temp, "%s%s", temp, combine[j]);
        //     printf("%s\n", temp);
        // }
        // n = length;
...
int main(){
    ...
    segregate_string_mark_half(passage, "as");
}